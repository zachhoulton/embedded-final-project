...
// Draw a partial subregion of a full image onto the screen.
// Parameters:
//   screenX, screenY: top-left corner on the screen where the region will be drawn
//   image: pointer to the full image (stored as a 1D array of uint16_t)
//   imageWidth, imageHeight: dimensions of the full image in ROM (e.g. 128 x 160)
//   srcX, srcY: starting coordinate (column, row) in the full image to begin drawing
//   boxWidth, boxHeight: the width and height of the subregion (box) to draw on the screen
void ST7735_DrawBitmapPartial(int16_t screenX, int16_t screenY, 
                              const uint16_t *image, 
                              int16_t imageWidth, int16_t imageHeight,
                              int16_t srcX, int16_t srcY,
                              int16_t boxWidth, int16_t boxHeight) {
    // Check that the source region falls within the image boundaries.
    if (srcX < 0 || srcY < 0 || srcX + boxWidth > imageWidth || srcY + boxHeight > imageHeight) {
        // Optionally handle error (source box out of image bounds)
        return;
    }
    
    // Determine the starting index in the image array corresponding to (srcX, srcY)
    int32_t i = srcY * imageWidth + srcX;

    // Clip the drawing window against the display boundaries.
    // Assume _width and _height are the screen dimensions.
    if ((screenX >= _width) || (screenY >= _height) ||
        ((screenX + boxWidth) <= 0) || ((screenY + boxHeight) <= 0)) {
        // The region is completely off-screen.
        return;
    }
    
    int16_t drawWidth = boxWidth;
    int16_t drawHeight = boxHeight;
    
    // Clip the region on the right if necessary
    if (screenX + drawWidth > _width) {
        drawWidth = _width - screenX;
    }
    // Clip the region on the bottom if necessary
    if (screenY + drawHeight > _height) {
        drawHeight = _height - screenY;
    }
    // If the drawing area goes off the left side, adjust the source index.
    if (screenX < 0) {
        int16_t delta = -screenX;   // number of columns off-screen
        drawWidth -= delta;
        i += delta;   // move the source pointer rightward by delta columns
        screenX = 0;
    }
    // If the drawing area goes off the top, adjust the source index.
    if (screenY < 0) {
        int16_t delta = -screenY;   // number of rows off-screen
        drawHeight -= delta;
        i += delta * imageWidth;    // move the source pointer down delta rows
        screenY = 0;
    }
    
    // Set up the display window.
    // Here we assume setAddrWindow(x0, y0, x1, y1) defines the drawing region.
    setAddrWindow(screenX, screenY, screenX + drawWidth - 1, screenY + drawHeight - 1);

    // Draw each row. Note that the source image rows are "imageWidth" pixels wide.
    for (int row = 0; row < drawHeight; row++) {
        int sourceIndex = i;
        for (int col = 0; col < drawWidth; col++) {
            uint16_t pixel = image[sourceIndex + col];
            // Send the 16-bit color value: first the high byte, then the low byte.
            SPI_OutData((uint8_t)(pixel >> 8));
            SPI_OutData((uint8_t)(pixel));
        }
...
void ST7735_DrawBitmapTransparent(int16_t x, int16_t y, const uint16_t *image, 
                                   int16_t w, int16_t h, int16_t sentinel, double intensity) {
  int16_t skipC = 0;              // columns to skip due to clipping
  int16_t originalWidth = w;      // original width stored in ROM
  int i = w * (h - 1);            // start at the top row (image stored bottom-up)

  // Clipping: if the image is completely off the screen, do nothing
  if ((x >= _width) || ((y - h + 1) >= _height) || ((x + w) <= 0) || (y < 0)) {
    return;
  }
  // If the image is larger than the screen, simply return
  if ((w > _width) || (h > _height)) {
    return;
  }
  // Clip right side if necessary
  if ((x + w - 1) >= _width) {
    skipC = (x + w) - _width; // columns to skip on right
    w = _width - x;
  }
  // Clip top side if necessary
  if ((y - h + 1) < 0) {
    i = i - (h - y - 1) * originalWidth;  // adjust index for skipped top rows
    h = y + 1;
  }
  // Clip left side if necessary
  if (x < 0) {
    w = w + x;
    skipC = -x;             // columns to skip on left
    i = i - x;              // adjust index for skipped left columns
    x = 0;
  }
  ...
  // Calculate the display Y coordinate of the top of the image
  int startY = y - h + 1;

  // Loop through the image pixels row-by-row.
  for (int row = 0; row < h; row++) {
    for (int col = 0; col < w; col++) {
      ...
      // Extract RGB components from the sentinel color
      uint8_t r2 = (sentinel >> 11) & 0x1F;
      uint8_t g2 = (sentinel >> 5)  & 0x3F;
      uint8_t b2 =  sentinel        & 0x1F;

      // If any channel differs by more than the tolerance, draw the pixel.
      if ((abs(r1 - r2) > 2) || (abs(g1 - g2) > 4) || (abs(b1 - b2) > 4)) {
        color = ((int16_t)(b1 + (3 - b1)*intensity) << 11) | ((int16_t)(g1 + (22 - g1)*intensity) << 5) | (int16_t)(r1 + (30 - r1)*intensity);
        
        ST7735_DrawPixel(x + col, startY + row, color);
      }
      i++;  // Move to next pixel in the current row
    }
    // Adjust index to account for any columns that were skipped and for bottom-up storage
    i += skipC;
    i -= 2 * originalWidth;
  }
}
...
//Fade
// Helper: Fade a single pixel from black toward its original color.
static inline uint16_t fadeInPixel(uint16_t color, float fadeAlpha) {
    // Extract the RGB components (RGB565 format)
    uint8_t r = (color >> 11) & 0x1F; // 5-bit red
    uint8_t g = (color >> 5)  & 0x3F; // 6-bit green
    uint8_t b =  color        & 0x1F; // 5-bit blue

    // Multiply each channel by the fade factor
    // When fadeAlpha == 0.0, r, g, b become 0 (black)
    // When fadeAlpha == 1.0, the original color is restored.
    r = (uint8_t)(r * fadeAlpha);
    g = (uint8_t)(g * fadeAlpha);
    b = (uint8_t)(b * fadeAlpha);

    // Reassemble into 16-bit RGB565 format
    return (r << 11) | (g << 5) | b;
}

// Modified bitmap drawing function that applies the fade factor to each pixel.
void ST7735_DrawBitmapFade(int16_t x, int16_t y, const uint16_t *image, 
                           int16_t w, int16_t h, float fadeAlpha) {
    int16_t skipC = 0;                      // columns to skip due to clipping
    int16_t originalWidth = w;              // width of the image in ROM
    int i = w * (h - 1);
    ...
    // For simplicity, we assume the image fits the screen.
    if ((w > _width) || (h > _height)) {
        return;
    }
    if ((x + w - 1) >= _width) {
    ...
    // Set the address window on the display.
    setAddrWindow(x, y - h + 1, x + w - 1, y);

    // Draw the bitmap line by line
    for (int row = 0; row < h; row++) {
        for (int col = 0; col < w; col++) {
            // Apply the fade factor to each pixel.
            uint16_t pixel = fadeInPixel(image[i], fadeAlpha);
            SPI_OutData((uint8_t)(pixel >> 8));  // send high 8 bits
            SPI_OutData((uint8_t)pixel);         // send low 8 bits
            i++;  // move to next pixel
        }
        // Adjust the index for any clipped columns and move up one row.
        i = i + skipC - 2 * originalWidth;
    }
    // Optionally deselect the display if needed.
    // deselect();
}

// Fade in the bitmap from black to full color over 1 second.
// This function calls the fade-drawing function repeatedly, increasing the fade factor.
void ST7735_FadeInBitmap(int16_t x, int16_t y, const uint16_t *image, 
                         int16_t w, int16_t h) {
    const int frames = 50;       // Assuming 60 frames per second.
    const int frameDelay = 1000 / frames; // Delay in milliseconds per frame.

    // Loop from 0 to frames, gradually increasing fadeAlpha from 0.0 to 1.0.
    for (int frame = 0; frame <= frames; frame++) {
        float fadeAlpha = frame / (float)frames;  // Linear interpolation.
        // Redraw the bitmap with the current fade value.
        ST7735_DrawBitmapFade(x, y, image, w, h, fadeAlpha);
        // Wait for the next frame.
        Clock_Delay1ms(frameDelay);
    }
}


//Fade-In Transparently
// This function draws a bitmap with fade applied to non-black pixels only.
// It uses per-pixel drawing so that any pixel whose original color is black is left unchanged.
void ST7735_DrawBitmapFadeTransparent(int16_t x, int16_t y, const uint16_t *image, 
                                        int16_t w, int16_t h, float fadeAlpha, int16_t sentinal) {
    int16_t skipC = 0;         // columns to skip due to clipping
    int16_t originalWidth = w; // the full image width stored in ROM
    int i = w * (h - 1);       // starting index (image stored bottom-up)

    // Clipping checks: if the image is completely off-screen or too big, exit.
    if ((x >= _width) || ((y - h + 1) >= _height) || ((x + w) <= 0) || (y < 0)) {
        return;
    }
    if ((w > _width) || (h > _height)) {
    ...
    // Calculate the top Y coordinate on the display.
    int startY = y - h + 1;

    // Loop through each pixel in the clipped image.
    for (int row = 0; row < h; row++) {
        for (int col = 0; col < w; col++) {
            uint16_t origPixel = image[i];
            // Only process non-black pixels.
            if (origPixel != sentinal) {
                // Apply the fade factor.
                uint16_t pixel = fadeInPixel(origPixel, fadeAlpha);
                // Draw the pixel at the corresponding display coordinates.
                ST7735_DrawPixel(x + col, startY + row, pixel);
            }
            i++; // Advance to the next pixel in the row.
        }
        // Adjust the index for any clipped columns and move to the next row (remember, image is stored bottom-up).
        i += skipC;
        i -= 2 * originalWidth;
    }
}

// This function animates the fade-in effect for a transparent bitmap.
// It repeatedly calls the fade-drawing function with gradually increasing fade values.
void ST7735_FadeInBitmapTransparent(int16_t x, int16_t y, const uint16_t *image, 
                                    int16_t w, int16_t h, int16_t sentinal) {
    const int frames = 50;                   // Number of frames in the fade animation.
    const int frameDelay = 1000 / frames;      // Delay per frame (in milliseconds).

    // Animate the fade from completely black (fadeAlpha=0.0) to full color (fadeAlpha=1.0).
    for (int frame = 0; frame <= frames; frame++) {
        float fadeAlpha = frame / (float)frames; // Linear interpolation for fade.
        ST7735_DrawBitmapFadeTransparent(x, y, image, w, h, fadeAlpha, sentinal);
        Clock_Delay1ms(frameDelay);              // Wait before drawing the next frame.
    }
}
